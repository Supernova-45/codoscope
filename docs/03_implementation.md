...
**3. The organism switch (THE HERO — make this excellent).** A prominent control (segmented buttons or a dropdown) over `organisms[]`. The encoded amino acid from the fixed DNA input stays constant by construction, but the model's amino-acid probability distribution does not necessarily stay put. Animate and quantify both. Show the within-observed-family synonymous distributions aligned side by side, together with observed-AA support in both organisms; never present a large conditional synonym shift without that calibration. Treat the taxid switch as a model-input counterfactual on a fixed prefix, not proof of a biological causal effect. Use a subtle "Δ" badge on positions whose top conditional synonym changes.
